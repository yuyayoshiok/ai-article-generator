...
def generate_article():
    try:
        ...
        あなたは高品質記事作成のエージェントです。以下のフレームワークに従って、キーワード「{keyword}」について完全な記事を作成してください。
        ...
        読者の心に響く導入を以下の手法で作成：
        - 読者の共感を呼ぶ問いかけから開始
        - 読者の好奇心を刺激する衝撃的な事実やデータを提示
        - 読者が抱える具体的な問題を明確に指摘
        - 記事を読むことで得られる具体的なベネフィットを明示
        - ストーリー要素や比喩表現を効果的に活用
        ...
        - 関連する統計データや研究結果を引用
        - 専門家の見解や公的機関のデータを活用
        - 読者が「なるほど、だからこの問題が重要なのか」と納得できる説明
        - 具体的な事例やケーススタディを紹介
        
        ### 3. 【解決策の提示】（全体の30%）
        3つのアプローチで解決策を提示：
        - **直接的解決策**: すぐに実行できる具体的な方法
        - **段階的解決策**: ステップバイステップで取り組める方法
        - **根本的解決策**: 長期的な視点での抜本的改善方法
        ...
        - 最初の一歩として今すぐできること
        - 未来への展望と励ましのメッセージ
        - 読者のコメントや体験談を歓迎する呼びかけ
        
        【文章作成の技術的要件】
        - Markdown記法を使用（見出しはH1〜H5で階層化）
        - 各セクションに明確で魅力的な見出しを設定
        - 重要な情報は箇条書きで整理
        - データは具体的な数字で示す（「多くの人が」→「調査対象の72%が」）
        - 専門用語は必ず平易な言葉で説明
        - 1段落3-4文、1文20-30文字を基本とする
        - 接続詞を効果的に使用（「しかし」「さらに」「つまり」「一方で」）
        - 読者への問いかけを適度に挿入して対話感を演出
        
        【プロフェッショナル要素の組み込み】
        - 権威性: 専門家の発言や公的機関のデータを引用
        - 信頼性: 可能な限り出典を明記し、客観的な分析を提供
        - 実用性: 読者がすぐに実践できる具体的なアドバイス
        - 独自性: 他では得られない体験談や独自の視点
        
        上記のフレームワークに従って、読者に真の価値を提供する完全な記事を作成してください。
        """
